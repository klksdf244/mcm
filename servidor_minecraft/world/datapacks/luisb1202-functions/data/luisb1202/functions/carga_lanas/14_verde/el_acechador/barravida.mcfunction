...
execute store result score @s danom run data get entity @e[tag=14_acechador_hitbox,scores={14_id=0},limit=1] Health

scoreboard players set @s danom3 100
scoreboard players operation @s danom *= @s danom3

scoreboard players operation @s danom /= @e[tag=14_acechador_hitbox,scores={14_id=0},limit=1] danom2
...
execute at @s run playsound block.netherite_block.hit master @a ~ ~ ~ 1 0.7
execute at @s run particle minecraft:item smooth_stone_slab ~ ~-1 ~ 0 0 0 0.4 20
