...
summon armor_stand 1024 174 -455 {NoGravity:1b,Silent:1b,Invulnerable:1b,Small:1b,Marker:1b,Invisible:1b,Tags:["100_luz_verde_as","100_luz_as","100_luz_as_ini"]}
execute as @e[tag=100_luz_as,tag=!100_luz_as_ini] at @s run tp @s ~ ~ ~1
tag @e[tag=100_luz_as_ini] remove 100_luz_as_ini

execute as @a[scores={id_lana=102}] run title @s title {"translate":"empty"}
execute as @a[scores={id_lana=102}] run title @s subtitle {"translate":"luisb1202.functions.carga_lanas.100_maquina_diamantes.fase_replica.pisar_verde.1"}